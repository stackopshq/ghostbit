...
_PATTERNS: list[tuple[str, re.Pattern]] = [
    ...
    ("css",        re.compile(r"\b(?:color|margin|padding|font-|background|display|border|width|height|flex|grid)\s*:", re.I)),
    # YAML front matter or key: value blocks
    ("yaml",       re.compile(r"^---\s*\n|^[a-z_][a-z0-9_]*:\s+\S", re.M)),
    # TypeScript (must come before JavaScript — stricter markers)
    ("typescript", re.compile(r":\s*(?:string|number|boolean|void|any|never)\b|interface\s+\w+\s*\{|type\s+\w+\s*=")),
    # JavaScript
    ("javascript", re.compile(r"\bconsole\.[a-z]+\s*\(|require\s*\(|(?:const|let|var)\s+\w+\s*=|=>\s*[{\w]")),
    # Python
    ("python",     re.compile(r"^\s*(?:def |class |import |from \w+ import |elif |@\w+)\b|print\s*\(", re.M)),
    # Go
    ...
    # Ruby
    ("ruby",       re.compile(r"^\s*(?:def |end\b|require ['\"]|puts |attr_(?:accessor|reader|writer))", re.M)),
...
_ALIAS_MAP: dict[str, str] = {
    ...
    "go": "go", "html": "html", "java": "java",
    "js": "javascript", "javascript": "javascript",
    "json": "json", "kotlin": "kotlin", "lua": "lua",
    "make": "makefile", "makefile": "makefile",
    "markdown": "markdown", "md": "markdown",
    "php": "php", "python": "python", "python3": "python", "py": "python",
    "rb": "ruby", "ruby": "ruby", "rust": "rust",
